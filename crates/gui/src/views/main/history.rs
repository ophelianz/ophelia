...
use crate::engine::{ArtifactState, HistoryFilter, TransferId, TransferStatus};
...
impl HistoryRowModel {
    fn from_row(row: HistoryListRow) -> Self {
        let (status_icon, status_color) = match row.status {
            TransferStatus::Finished => (IconName::CircleCheck, Colors::active().into()),
            TransferStatus::Error => (IconName::CircleX, Colors::error().into()),
            TransferStatus::Cancelled => (IconName::CircleX, Colors::muted_foreground().into()),
            TransferStatus::Paused => (IconName::CirclePause, Colors::queued().into()),
            TransferStatus::Downloading => (IconName::ArrowDownToLine, Colors::finished().into()),
