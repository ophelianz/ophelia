...
use crate::engine::{TransferId, TransferStatus};
...
impl TransferFilter {
    fn matches(self, status: TransferStatus) -> bool {
        match self {
            ...
            Self::Finished => status == TransferStatus::Finished,
            Self::Paused => status == TransferStatus::Paused,
            Self::Failed => matches!(status, TransferStatus::Error | TransferStatus::Cancelled),
        }
    }
}
...
impl TransferList {
    ...
    fn set_selected_id(&mut self, selected_id: Option<TransferId>, cx: &mut Context<Self>) {
...
mod tests {
    ...
    fn selection_defaults_to_first_visible_row() {
        let rows = vec![test_row(TransferId(3)), test_row(TransferId(7))];
    ...
    fn selection_preserves_current_row_when_still_visible() {
        ...
        assert_eq!(
            resolve_selected_transfer_id(&rows, Some(TransferId(7))),
            Some(TransferId(7))
        );
    }
    ...
    fn selection_falls_back_when_row_is_removed_or_filtered_out() {
        let rows = vec![test_row(TransferId(11)), test_row(TransferId(14))];

        assert_eq!(
            resolve_selected_transfer_id(&rows, Some(TransferId(99))),
            Some(TransferId(11))
        );
    }
    ...
    fn selection_clears_when_no_rows_are_visible() {
        assert_eq!(resolve_selected_transfer_id(&[], Some(TransferId(5))), None);
    }
