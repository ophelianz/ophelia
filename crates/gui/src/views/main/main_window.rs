...
pub struct MainWindow {
    ...
    selected_transfer_id: Option<TransferId>,
...
mod tests {
    ...
    use crate::engine::TransferStatus;

    #[test]
    fn chunk_map_selection_defaults_to_first_visible_transfer() {
        let rows = vec![test_row(TransferId(4)), test_row(TransferId(9))];
    ...
    fn chunk_map_selection_falls_back_when_current_transfer_disappears() {
        let rows = vec![test_row(TransferId(12)), test_row(TransferId(15))];

        assert_eq!(
            resolve_selected_transfer_id_for_transfers(&rows, Some(TransferId(99))),
            Some(TransferId(12))
        );
    }
